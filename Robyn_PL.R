robyn_update()
#forecast conversions for 2 months, take out conversions from the input 
...
dat<- read.csv("~/Downloads/PL_Google_weekly_enough.csv", header = TRUE, sep = ';')
...
dat <- na.omit(dat)
#log
dat$YT_Impressions <- as.numeric(dat$YT_Impressions)
dat$YT_Clicks <- as.numeric(dat$YT_Clicks)
dat$YT_Cost <- as.numeric(dat$YT_Cost)
dat$FB_Impressions <- as.numeric(dat$FB_Impressions)
dat$FB_Clicks <- as.numeric(dat$FB_Clicks)
dat$FB_Cost <- as.numeric(dat$FB_Cost)
dat$Conversion_Revenue <- as.numeric(dat$Conversion_Revenue)
dat$Users <- as.numeric(dat$Users)

typeof(dat$YT_Impressions)
...
InputCollect <- robyn_inputs(
  ...
  prophet_country = "PL", # input one country. dt_prophet_holidays includes 59 countries by default
...
#correlations
sd_fb <- sd(dat$FB_Impressions)
sd_yt <- sd(dat$YT_Impressions)
cor(dat$FB_Impressions,dat$FB_Cost)
cor(dat$YT_Clicks,dat$YT_Cost)
...
sum(dat$FB_Cost)
sum(dat$YT_Cost)
sum(dat$Conversion_Revenue)
?robyn_inputs
calibration_input <- data.frame(
  #channel name must in paid_media_vars
  channel = c('FB_Cost','YT_Cost'),
  # liftStartDate must be within input data range
  liftStartDate = as.Date(c("2021-01-17","2021-01-17")),
  # liftEndDate must be within input data range
  liftEndDate = as.Date(c("2022-10-30","2022-10-30")),
  # Provided value must be tested on same campaign level in model and same metric as dep_var_type
  liftAbs = c(1368112, 1368112),#the amount of Conversion.Revenue attributed to each channel,80% because it's PL
  # Spend within experiment: should match within a 10% error your spend on date range for each channel from dt_input
  spend = c(150531, 1527923),
  # Confidence: if frequentist experiment, you may use 1 - pvalue
  confidence = c(0.91, 0.9),
...
if (length(InputCollect$exposure_vars) > 0) {
  ...
  InputCollect$modNLS$plots$YT_Clicks
}
...
print(OutputCollect)
  select_model <- "1_389_7" # Pick one of the models from OutputCollect to proceed

#### Since 3.7.1: JSON export and import (faster and lighter than RDS files)
ExportedModel <- robyn_write(InputCollect, OutputCollect, select_model)
print(ExportedModel)

one_page <- robyn_onepagers(
  InputCollect,
  OutputCollect,
...
one_page
one_page$`1_389_7`$patches$plots[[1]]
